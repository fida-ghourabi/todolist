...
import { FaTasks } from 'react-icons/fa'; // Ajoutez une icône si vous voulez
...
const [taskToDelete, setTaskToDelete] = useState(null);
  ...
  const fetchTasks = async () => {
    try {
      const response = await axios.get('http://localhost:8000/api/tasks/status/pending', {
        headers: {
  ...
  return (
    <div>
      <h2 className="h2">Pending Tasks</h2>
      {tasks.length === 0 ? (
        <div className="text-center py-5">
          <FaTasks size={80} className="text-muted mb-3" />
          <p className="text-muted">No pending tasks available</p>
        </div>
      ) : (
