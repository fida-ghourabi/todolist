...
const fetchTasks = async () => {
  try {
    const response = await axios.get('http://localhost:8000/api/tasks/status/started', {
      headers: {
...
const handleDelete = async (id) => {
  ...
  return (
    <div>
      <h2 className="h2">Started Tasks</h2>
      {tasks.length === 0 ? (
        <div className="text-center py-5">
          <FaTasks size={80} className="text-muted mb-3" />
          <p className="text-muted">No started tasks available</p>
        </div>
      ) : (
